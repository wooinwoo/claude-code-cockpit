...
import { calcCost, aggregateEntries, resolvePricing, PRICING, DEFAULT_PRICING } from '../../lib/cost-service.js';

describe('resolvePricing (family-based)', () => {
  it('resolves opus / sonnet / haiku families by substring', () => {
    assert.strictEqual(resolvePricing('claude-opus-4-8').p.output, 75);
    assert.strictEqual(resolvePricing('claude-sonnet-4-6').p.output, 15);
    assert.strictEqual(resolvePricing('claude-haiku-4-5-20251001').p.output, 4);
  });

  it('regression: sonnet is NOT priced at the opus fallback (the 5x bug)', () => {
    // A stale exact-match table silently priced sonnet-4-6 at opus ($75) — 5x over.
    const sonnet = resolvePricing('claude-sonnet-4-6');
    assert.strictEqual(sonnet.p.output, 15);
    assert.strictEqual(sonnet.estimated, false);
  });

  it('unknown family (e.g. fable) uses opus proxy and is flagged estimated', () => {
    const fable = resolvePricing('claude-fable-5');
    assert.deepStrictEqual(fable.p, DEFAULT_PRICING);
    assert.strictEqual(fable.estimated, true);
  });

  it('exact PRICING override wins over family match', () => {
    // PRICING starts empty; a precise override, if added, must take precedence.
    PRICING['test-model-x'] = { input: 1, output: 2, cacheWrite: 3, cacheRead: 4 };
    try {
      assert.deepStrictEqual(resolvePricing('test-model-x').p, PRICING['test-model-x']);
    } finally {
      delete PRICING['test-model-x'];
    }
  });

  it('DEFAULT_PRICING matches the opus tier', () => {
    assert.deepStrictEqual(DEFAULT_PRICING, { input: 15, output: 75, cacheWrite: 18.75, cacheRead: 1.5 });
  });
});
...
describe('getCostInsights', () => {
  it('returns a well-formed insight shape', async () => {
    const { getCostInsights } = await import('../../lib/cost-service.js');
    const r = await getCostInsights();
    assert.strictEqual(typeof r.date, 'string');
    assert.strictEqual(typeof r.todayCost, 'number');
    assert.ok(r.todayCost >= 0);
    assert.ok(r.burnPerHour >= 0);
    assert.ok(Array.isArray(r.bySession));
    assert.ok(Array.isArray(r.byModel));
    assert.ok(r.savingsCeiling >= 0, 'savings ceiling is non-negative');
    assert.strictEqual(typeof r.savingsNote, 'string');
  });
});
