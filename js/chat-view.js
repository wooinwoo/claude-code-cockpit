...
async function refreshChat(leaf) {
  ...
  try {
    ...
    if (msgsEl) {
      ...
      if (fullRender) {
        // 세션 변경 또는 초기 → 전체 재렌더
        msgsEl.innerHTML = renderMessagesNew(newMsgs);
      } else {
...
document.addEventListener('click', async (e) => {
  ...
  if (codeCopy) {
    ...
    if (raw) { await copyToClipboard(raw); flashButton(codeCopy, '✓ 복사됨'); }
    return;
  }
  // 코드 블록 펼치기/접기
  ...
  if (msgCopy) {
    ...
    if (raw) { await copyToClipboard(raw); flashButton(msgCopy, '✓'); }
    return;
  }
  // 메시지 접기/펼치기
