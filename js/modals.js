...
import { copyText, esc, escapeHtml, showToast, timeAgo, fuzzyMatch, IMG_EXT, fetchJson, postJson } from './utils.js';
...
function buildCommandList() {
  ...
  cmds.push({ group: 'Navigation', label: 'Go to PRs', hint: 'Ctrl+4', icon: gitIcon, action: () => notify('switchView', 'pr') });
  ...
  cmds.push({ group: 'Navigation', label: 'Go to Jira', hint: 'Ctrl+5', icon: jiraIcon, action: () => notify('switchView', 'jira') });
  const cicdIcon = icon('<circle cx="12" cy="12" r="10"/><path d="M8 12l2 2 4-4"/>');
  cmds.push({ group: 'Navigation', label: 'Go to CI/CD', hint: 'Ctrl+6', icon: cicdIcon, action: () => notify('switchView', 'cicd') });
  const notesIcon = icon('<path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/>');
  cmds.push({ group: 'Navigation', label: 'Go to Notes', hint: 'Ctrl+7', icon: notesIcon, action: () => notify('switchView', 'notes') });
  const logsIcon = icon('<path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/>');
  cmds.push({ group: 'Navigation', label: 'Go to Workflows', hint: 'Ctrl+8', icon: logsIcon, action: () => notify('switchView', 'workflows') });
...
export function copyFilePathToClipboard() {
  const el = document.getElementById('file-preview-overlay');
  if (el?._filePath) copyText(el._filePath).then((ok) => showToast(ok ? 'Path copied' : 'Copy failed', ok ? 'success' : 'error'));
}
export function copyFileContent() {
  const el = document.getElementById('file-preview-overlay');
  if (el?._content) copyText(el._content).then((ok) => showToast(ok ? 'Content copied' : 'Copy failed', ok ? 'success' : 'error'));
}
export function copyFileAsCodeBlock() {
  ...
  const block = '```' + ext + '\n' + el._content + '\n```';
  copyText(block).then((ok) => showToast(ok ? 'Copied as code block' : 'Copy failed', ok ? 'success' : 'error'));
}
