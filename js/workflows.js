...
import { copyText, esc, showToast, timeAgo, simpleMarkdown, fetchJson, postJson } from './utils.js';
...
export function copyStepOutput(runId, stepId) {
  ...
  if (body) {
    copyText(body.textContent).then((ok) => showToast(ok ? 'Copied' : 'Copy failed', ok ? 'success' : 'error'));
  }
}
