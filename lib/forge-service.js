...
DANGEROUS FILES — these files MUST use "edits" format, NEVER "new_files":
- router.tsx, App.tsx, main.tsx — 전체 교체 시 앱이 깨짐. 반드시 edits로 부분 수정만.
- vite.config.ts, tsconfig.json, package.json — 설정 파일도 edits로만.
- 이미 존재하는 모든 파일 → edits 사용. new_files는 새로 만드는 파일에만!
