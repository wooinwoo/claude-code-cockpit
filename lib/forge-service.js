...
export function getForgeRun(taskId) {
  ...
  return {
    ...
    designContext: run.designContext ? { screenshots: (run.designContext.screenshots || []).map(s => ({ page: s.page, route: s.route, data: s.data ? true : false })) } : null,
    error: run.error,
  };
}
...
async function _runForge(run) {
  ...
  try {
    ...
    if (plan && plan.batches && plan.batches.length > 1) {
      // Multi-batch pipeline: Pencil Design → Scaffold → Batch Build → Integrate → Verify
      ...
      // Simple task (single component / small change) — original flow
      run.updatePhase('building', 'building');
...
If the task is SIMPLE (1-3 files, single component, small fix), output:
{"scale":"simple"}

If the task is LARGE (multi-page app, full feature, 4+ files), output a full plan:
{
  "scale": "large",
...
${PROMPT_SECURITY}`;
  ...
  if (!response || response.scale === 'simple') {
    run.addLog('system', 'Design: simple task — skipping multi-batch pipeline');
    return null;
  }
...
async function _tryPencilDesign(run, plan) {
  try {
    ...
    run.addLog('system', 'Pencil connected — generating design...');
    const result = await pencil.designPages(plan, IS_WIN ? toWinPath(run.projectPath) : run.projectPath);

    if (result) {
      pencil.stop(); // Release Pencil after use
      return result;
    }
    return null;
  } catch (e) {
...
DESIGN TOKENS:
- Primary color: ${tokens.primaryColor || '#3182F6'}
- Style: ${tokens.style || 'minimal-toss'}
