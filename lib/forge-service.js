  ...
  for (const file of run.finalFiles) {
    ...
    try {
      ...
      if (file.action === 'modify' && file.changes) {
        ...
        // Safety: if file exists and LLM used new_files instead of edits, log warning
        if (existsSync(fullPath)) {
          ...
          // Check if LLM output is suspiciously small vs original (likely bad overwrite)
          if (content.length < original.length * 0.3 && original.length > 500) {
            run.addLog('system', `BLOCKED overwrite of ${file.path} (${content.length} chars vs original ${original.length} chars — too much code removed)`);
            continue; // Block destructive overwrites
          }
