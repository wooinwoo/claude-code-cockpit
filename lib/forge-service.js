...
async function _runForge(run) {
  ...
  try {
    ...
    if (plan && plan.batches && plan.batches.length > 1) {
      // Multi-batch pipeline: Scaffold → Batch Build → Integrate → Verify
...
6. Layout component (header with navigation, footer)
...
2. Import shared components from existing files (ProductCard, CartBadge, etc.)
