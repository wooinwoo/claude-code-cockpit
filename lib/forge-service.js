...
class ForgeRun {
  constructor(taskId, projectId, projectPath, task, referenceFiles, mode, costLimit) {
    ...
    this.error = null;
    this._stopped = false;
    this.awaitingApproval = null; // null | 'plan' | 'design'
...
export async function chatAboutPlan(taskId, message, history = []) {
  const run = _runs.get(taskId);
  if (!run) throw new Error(`Forge run not found: ${taskId}`);
  if (!run.design) throw new Error('No design plan available yet');

  const planJSON = JSON.stringify(run.design, null, 2);
