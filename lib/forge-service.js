...
const MAX_CONCURRENT_RUNS = 10; // Allow many parallel runs across different projects
...
export async function startForge({ projectId, projectPath, task, referenceFiles, mode, costLimit, pipeline, issueKey }) {
  ...
  const activeRuns = [..._runs.values()].filter(r => !['done', 'failed', 'stopped', 'cost_exceeded'].includes(r.status));
  ...
  // Same project: only allow 1 active fixing run (analysis is OK in parallel)
  const sameProjectFixing = activeRuns.filter(r => r.projectPath === projectPath && r.pipeline === 'fix' && ['fixing', 'building', 'verifying', 'committing'].includes(r.phase));
  if (sameProjectFixing.length > 0) {
    // Queue it — analysis can proceed, but fixing waits
    // For now, just allow it — the builder will handle sequential execution
  }
  ...
  try {
    ...
    const branchName = run.pipeline === 'fix'
      ? `fix/${run.issueKey || taskId}`
      : `forge/${taskId}`;
...
export function getForgeRun(taskId) {
  ...
  return {
    ...
    finalFiles: run.finalFiles.map(f => ({ path: f.path, action: f.action, content: f.content || null, changes: f.changes || null })),
...
export function listForgeRuns() {
  return [..._runs.values()].map(r => ({
    taskId: r.taskId, projectId: r.projectId, projectPath: r.projectPath,
    task: r.task, status: r.status, phase: r.phase,
    awaitingApproval: r.awaitingApproval || null,
    pipeline: r.pipeline, issueKey: r.issueKey,
    mode: r.mode, startedAt: r.startedAt, endedAt: r.endedAt,
    cost: r.cost.total, finalFiles: r.finalFiles?.length || 0,
    prUrl: r.prUrl || null, error: r.error || null,
...
async function _resumeAfterFixApproval(run) {
  ...
  // Phase 2: Fix code (minimal edit approach, not full rewrite)
  run.updatePhase('fixing', 'building');
  await _runFixBuilder(run);
  if (run.isStopped()) return finalize(run);
  _writeFilesToDisk(run);
...
async function _resumeAfterDiffApproval(run) {
  ...
  try {
    ...
    // Git add + commit (use temp file for commit message to handle Korean/special chars)
    const commitMsg = `fix(${run.issueKey || 'fix'}): ${(run.task || '').slice(0, 60)}`;
    const commitMsgFile = join(DATA_DIR, '.commit-msg-tmp');
    writeFileSync(commitMsgFile, commitMsg, 'utf8');
    execSync(`git add -A`, { cwd: nativePath, encoding: 'utf8' });
    execSync(`git commit -F "${IS_WIN ? toWinPath(commitMsgFile) : commitMsgFile}"`, { cwd: nativePath, encoding: 'utf8' });
    try { unlinkSync(commitMsgFile); } catch { /* cleanup */ }
    ...
    // Create PR via gh CLI (use --body-file to handle Korean/special chars)
    try {
      const prTitle = commitMsg.replace(/["`$\\]/g, '');  // sanitize for shell
      const prBody = `## ${run.issueKey || 'Fix'}\n\n${run.analysis?.rootCause || ''}\n\n### Changes\n${run.finalFiles.map(f => '- ' + f.path).join('\n')}`;
      const prBodyFile = join(DATA_DIR, '.pr-body-tmp');
      writeFileSync(prBodyFile, prBody, 'utf8');
      const bodyPath = IS_WIN ? toWinPath(prBodyFile) : prBodyFile;
      const prResult = execSync(`gh pr create --title "${prTitle}" --body-file "${bodyPath}"`, { cwd: nativePath, encoding: 'utf8', timeout: 30000 });
      run.prUrl = prResult.trim();
      run.addLog('system', `PR created: ${run.prUrl}`);
      try { unlinkSync(prBodyFile); } catch { /* cleanup */ }
    } catch (prErr) {
...
async function _runAnalysis(run) {
  const systemPrompt = `당신은 10년차 시니어 풀스택 개발자입니다. 버그/결함 티켓을 분석하고 수정 계획을 세워야 합니다.
코드베이스 컨텍스트와 티켓 설명을 분석하여, 구조화된 수정 계획을 JSON으로 출력하세요.

분석 시 반드시 포함할 사항:
1. 코드를 실제로 읽고 원인을 정확히 파악 (추측 금지)
2. 영향받는 파일을 전부 나열 (import 체인, 의존성 포함)
3. 수정 전략을 구체적인 코드 레벨로 설명 (어떤 함수의 몇 번째 줄, 어떤 로직)
4. 각 수정 단계를 상세히 — "무엇을 왜 어떻게" 형태로
5. 사이드이펙트 정직하게 평가
...
{
  "rootCause": "원인을 구체적으로. 어떤 파일의 어떤 함수에서 어떤 로직이 문제인지. 코드 라인까지 언급.",
  "affectedFiles": ["수정할 파일 전체 경로"],
  "impactScope": "low|medium|high",
  "impactDetail": "영향 범위 상세 설명. 어떤 기능/페이지에 영향이 가는지.",
  "fixStrategy": "수정 전략을 단계별로 상세히. 어떤 코드를 어떻게 바꿀지 구체적으로.",
  "steps": [
    {"file": "파일 경로", "action": "modify|create|delete", "description": "이 파일에서 무엇을 왜 어떻게 수정하는지 상세히", "codeHint": "수정할 코드 부분 힌트 (함수명, 변수명 등)"}
  ],
  "risksAndSideEffects": "사이드이펙트 정직하게. 없으면 '없음'.",
  "testStrategy": "수정 후 확인할 방법. 어떤 시나리오를 테스트해야 하는지.",
  "recommendedMode": "quick|standard|thorough — 수정 난이도에 따라 추천. 1-2파일 단순 수정=quick, 여러 파일+로직 변경=standard, 대규모+위험=thorough",
  "modeReason": "추천 이유 한 줄",
  "needsOtherRepo": false,
  "otherRepoReason": "이 태스크가 다른 레포(백엔드/프론트)의 변경도 필요한지. 필요하면 true + 이유. 예: API 엔드포인트가 없어서 백엔드에서 먼저 만들어야 함",
  "needsNewDependencies": false,
  "newDependencies": []
}`;

  const budget = _contextBudget('opus');
...
Analyze the code and produce the fix plan as JSON.`;
  ...
  const response = await callLLMWithJSON(run, 'system', systemPrompt, userPrompt, { model: 'opus' });
...
export function approveForge(taskId, type, options = {}) {
  const run = _runs.get(taskId);
  ...
  // Allow mode change at approval time
  if (options.mode && MODE_PRESETS[options.mode]) {
    run.mode = options.mode;
    run.addLog('system', `Mode changed to: ${options.mode}`);
  }
...
export function rejectForge(taskId, type, feedback) {
  ...
  if (type === 'plan') {
    ...
    // Diff rejection: re-run fix with feedback
    run.addLog('system', `Diff rejected — re-fixing: ${feedback || '(no feedback)'}`);
    run.task = run.task + '\n\n[추가 수정 요청] ' + (feedback || '');
    run.finalFiles = []; // clear previous fixes
    _resumeAfterFixApproval(run).catch(err => _handleResumeError(run, err));
...
export async function chatAboutPlan(taskId, message, history = [], fallbackPlan = null) {
  ...
  const context = analysis || design;
  if (!context) throw new Error('No analysis or plan data available');

  const contextJSON = JSON.stringify(context, null, 2);
  const isFix = run?.pipeline === 'fix';

  const systemPrompt = isFix
    ? `당신은 코드 분석 결과를 검토하는 시니어 개발자입니다. 사용자가 분석 결과에 대해 질문하거나 수정을 요청합니다.
...
원래 태스크: ${run?.task || ''}
프로젝트: ${run?.projectPath || ''}

규칙:
- 분석 결과에 대한 질문에 상세히 답변 (원인, 영향 범위, 수정 전략, 파일, 위험 요소 등)
- 코드 구조나 로직에 대해 설명할 때 구체적인 파일명과 함수명 포함
- 사용자가 분석 방향을 바꾸거나 추가 파일 확인을 원하면, 변경 내용 설명 후 메시지 끝에: [NEEDS_REDESIGN]
- 한국어로 답변. 마크다운 사용 가능. 코드 블록 사용 가능.`
    : `You are a service planner assistant. The user is reviewing a project plan before approving it for build.
...
- Do NOT wrap response in code blocks or JSON.`;
  ...
  const response = await _callClaude(userPrompt, {
    model: 'opus', systemPrompt, timeoutMs: 60000,
  });
...
// ── Fix Builder: Minimal edits to existing codebase (like Claude Code native) ──

async function _runFixBuilder(run) {
  const preset = MODE_PRESETS[run.mode] || MODE_PRESETS.standard;
  const analysis = run.analysis;
  ...
  const frameworkHint = run.framework === 'nestjs'
    ? 'This is a NestJS backend project. Follow NestJS patterns: modules, controllers, services, DTOs, guards, pipes. Use decorators properly.'
    : run.framework === 'nextjs' || run.framework === 'react'
    ? 'This is a React/Next.js frontend project. Follow React patterns: components, hooks, state management.'
    : 'Detect the project type from the code and follow its conventions.';

  const systemPrompt = `You are fixing code in an existing project. Make MINIMAL, targeted changes.
Framework: ${run.framework}. ${frameworkHint}

RULES:
1. For EXISTING files: use "edits" format with anchor + replacement. NEVER rewrite entire files.
2. For NEW files only: use "new_files" format.
3. "anchor" must be 3+ lines of EXACT existing code that uniquely identifies the edit location.
4. "replacement" is the new code replacing the anchor.
5. Keep all existing code intact — only change what's needed for the fix.
6. All text/comments in Korean.
7. Follow the existing code style exactly (indentation, naming, patterns).

OUTPUT FORMAT (JSON):
{
  "status": "complete",
  "summary": "변경 요약 (한국어)",
  "new_files": [{"path": "새파일.tsx", "content": "전체 내용"}],
  "edits": [{"path": "기존파일.tsx", "changes": [{"anchor": "기존 코드 3줄 이상", "replacement": "수정된 코드", "description": "변경 설명"}]}]
}

ANALYSIS RESULTS:
- Root cause: ${analysis?.rootCause || 'N/A'}
- Affected files: ${analysis?.affectedFiles?.join(', ') || 'N/A'}
- Fix strategy: ${analysis?.fixStrategy || 'N/A'}
- Steps: ${JSON.stringify(analysis?.steps || [])}`;

  const userPrompt = `TASK: ${run.task}

PROJECT FILES:
${projectContext}

Apply the fix. Use "edits" for existing files, "new_files" only for brand new files. Output JSON.`;

  run.addLog('builder', 'Fix builder starting (minimal edit mode)');
  const response = await callLLMWithJSON(run, 'builder', systemPrompt, userPrompt, { model: preset.builderModel });

  if (!response) {
    run.addLog('builder', 'Fix builder returned no response');
    return;
  }

  run.addLog('builder', `Fix complete: ${response.summary || 'done'}`);
...
function _writeFilesToDisk(run) {
  ...
  for (const file of run.finalFiles) {
    ...
    try {
      ...
      if (file.action === 'modify' && file.changes) {
        ...
        // Safety: if file exists and LLM used new_files instead of edits
        if (existsSync(fullPath)) {
          const original = readFileSync(fullPath, 'utf8');
          if (original.trim() === content.trim()) {
            run.addLog('system', `Skipped unchanged file: ${file.path}`);
            continue;
          }
          // Critical files: block overwrite entirely, only edits allowed
          const criticalFiles = ['router.tsx', 'App.tsx', 'main.tsx', 'vite.config.ts'];
          if (criticalFiles.some(cf => file.path.endsWith(cf))) {
            run.addLog('system', `BLOCKED overwrite of critical file: ${file.path} (use edits instead)`);
            continue;
          }
          // Check if LLM output removes too much code
          if (content.length < original.length * 0.3 && original.length > 500) {
            run.addLog('system', `BLOCKED destructive overwrite of ${file.path} (${content.length} vs ${original.length} chars)`);
            continue;
          }
          run.addLog('system', `Overwriting existing file: ${file.path} (${original.length} → ${content.length} chars)`);
        }
        mkdirSync(join(fullPath, '..'), { recursive: true });
  ...
  // Run import reconciliation before verification (skip for fix pipeline — minimal changes only)
  if (run.pipeline !== 'fix') {
    try {
      reconcileImports({
...
async function _runVerificationSquad(run) {
  ...
  const isNest = run.framework === 'nestjs';
  const isReact = ['react', 'nextjs'].includes(run.framework);
  ...
  // 2. Build check (framework-specific)
  ...
  // 3. Lint (ESLint for all, auto-fix)
  ...
  // 4. Dead code / unused (Knip for React, skip for NestJS)
  if (isReact) {
    const knipResult = _runShellCheck(nativePath, 'npx knip 2>&1 || true', 'knip');
    results.push(knipResult);
    if (knipResult.errors.length > 0) run.addLog('verify:knip', `Knip: ${knipResult.errors.length} unused export(s)`);
  }

  // 5. Tests (NestJS: jest, React: skip unless configured)
  if (isNest) {
    const testResult = _runShellCheck(nativePath, 'npx jest --passWithNoTests --bail 2>&1 || true', 'test');
    results.push(testResult);
    run.addLog('verify:test', testResult.passed ? 'Tests passed' : `Tests: ${testResult.errors.length} failure(s)`);
  } else {
    // React: run tests only if test script exists
    try {
      const pkg = JSON.parse(readFileSync(join(nativePath, 'package.json'), 'utf8'));
      if (pkg.scripts?.test && !pkg.scripts.test.includes('no test')) {
        const testResult = _runShellCheck(nativePath, 'npm test -- --bail --passWithNoTests 2>&1 || true', 'test');
        results.push(testResult);
        run.addLog('verify:test', testResult.passed ? 'Tests passed' : `Tests: ${testResult.errors.length} failure(s)`);
      }
    } catch {}
...
TIP: Use "edits" for existing files when possible. If you use "new_files" for an existing file, include ALL the original code plus your changes — partial/truncated rewrites will be blocked.
...
- NEW files → use "new_files" (full content)
- EXISTING files → use "edits" (anchor + replacement). NEVER put existing files in new_files — this OVERWRITES the entire file and destroys other code!
- "anchor" must be 3+ lines of EXACT existing code that uniquely identifies the edit location
- "replacement" is the new code to replace the anchor
- For adding imports: anchor the existing import block, replacement includes old imports + new ones
- For adding routes: anchor the existing routes, replacement includes old routes + new one
- NEVER rewrite entire existing files — only change the specific lines that need modification
