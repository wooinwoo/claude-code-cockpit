...
import { gitExec, shellExec, parseWslPath, toWinPath } from './wsl-utils.js';
...
try { mkdirSync(FORGE_HISTORY_DIR, { recursive: true }); } catch {}
...
let _runs = new Map(); // runId → ForgeRun
...
// ── ForgeRun class ──
class ForgeRun {
  constructor(taskId, projectId, projectPath, task, referenceFiles, mode, modelPreset, costLimit) {
    this.taskId = taskId;
    ...
    this.status = 'pending'; // pending | designing | building | verifying | integrating | done | failed | stopped
    this.phase = null;
...
export async function startForge({ projectId, projectPath, task, referenceFiles, mode, modelPreset, costLimit }) {
  ...
  const run = new ForgeRun(taskId, projectId, projectPath, task, referenceFiles, mode, modelPreset, costLimit);
  ...
  _poller?.broadcast('forge:start', {
    taskId, projectId, task, framework, mode, modelPreset, costLimit
  });
...
export function getForgeRun(taskId) {
  ...
  return {
    ...
    mode: run.mode, modelPreset: run.modelPreset, costLimit: run.costLimit,
...
export async function applyForgeResult(taskId, projectPath) {
  ...
  const rootDir = normalize(toWinPath(projectPath)).toLowerCase().replace(/\\/g, '/');
  const results = [];
  ...
  for (const file of run.finalFiles) {
    ...
    try {
      const fullPath = resolve(toWinPath(projectPath), file.path);
...
export async function forgeReview({ projectId, projectPath, diff, files }) {
