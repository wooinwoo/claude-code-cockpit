...
// ─── 관제 타일 활동 요약 ───
const SUMMARY_SYSTEM = `너는 개발 관제 대시보드의 상태 요약기다. AI 코딩 에이전트가 돌아가는 터미널 출력의 마지막 부분을 받는다.
에이전트가 지금 무슨 작업을 하는 중인지 한국어 한 문장, 40자 이내로 요약해라.
파일명·기능명·에러명 같은 구체 맥락을 담고 "~중" 으로 끝내라 (예: "결제 라우트 테스트 실패 원인 수정 중").
판단할 맥락이 없으면 "대기 중" 이라고만 해라. 요약 문장만 출력하고 따옴표·마크다운·설명 금지.`;

export async function summarizeActivity(terminalText, timeoutMs = 15_000) {
  if (!_client) refreshClient();
  if (_client) {
    try {
      const text = await _client.send(SUMMARY_SYSTEM, terminalText, { timeoutMs });
      if (text?.trim()) return text.replace(/\s+/g, ' ').trim().slice(0, 80);
    } catch (e) {
      _logger.error?.('supervisor-llm', 'Gemini summarize failed', e.message);
    }
  }
  try {
    const { stdout } = await execFileAsync('claude', [
      '-p', '--model', 'haiku', '--safe-mode', '--tools', '',
      '--no-session-persistence', '--output-format', 'text',
      `${SUMMARY_SYSTEM}\n\n최근 터미널 출력:\n${terminalText}`,
    ], { timeout: timeoutMs, maxBuffer: 4096, windowsHide: true });
    return stdout.replace(/\s+/g, ' ').trim().slice(0, 80) || null;
  } catch (e) {
    _logger.error?.('supervisor-llm', 'Haiku summarize failed', e.message);
    return null;
  }
}
