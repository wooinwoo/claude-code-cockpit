...
export const PORT = Number.parseInt(process.env.PRAETORIUM_PORT || process.env.COCKPIT_PORT, 10) || 3847;
export const PROJECTS_ROOT = resolve(process.env.PRAETORIUM_PROJECTS_ROOT || process.env.COCKPIT_PROJECTS_ROOT || (process.platform === 'win32' ? 'C:\\projects' : join(homedir(), 'projects')));

const localAppData = process.env.LOCALAPPDATA || process.env.APPDATA || join(homedir(), '.local', 'share');
export const DATA_DIR = join(localAppData, 'Praetorium');
const legacyDataDir = join(localAppData, 'cockpit');
...
function load() {
  const candidates = [projectsFile, join(legacyDataDir, 'projects.json')];
