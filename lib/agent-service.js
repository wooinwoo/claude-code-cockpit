...
/** True when a Gemini key is loaded — gate side-effecting work (e.g. monitor reviews) on this. */
...
export function chat(convId, userMessage, targetAgentId = null, projectId = null) {
  ...
  if (projectId) {
    ...
    if (proj) {
      trimmedMessage = `[대상 프로젝트: ${proj.name} | 경로: ${proj.path} | 스택: ${proj.stack || '미정'}]\n\n${trimmedMessage}`;
    }
  }
  let lengthWarning = '';
