...
// ── opencode 세션 감지 (SQLite 저장소) ──
// COCKPIT_OPENCODE_DB: 테스트용 경로 오버라이드 (호출마다 재평가)
function opencodeDbPath() {
  return process.env.COCKPIT_OPENCODE_DB || join(homedir(), '.local', 'share', 'opencode', 'opencode.db');
}

const STATE_RANK = { busy: 3, waiting: 2, idle: 1 };

function opencodeSessionRow(db, projectPath) {
  const exact = db.prepare(
    'SELECT id, directory, model, time_updated FROM session WHERE directory = ? AND time_archived IS NULL ORDER BY time_updated DESC LIMIT 1',
  ).get(projectPath);
  if (exact) return exact;
  return db.prepare(
    'SELECT id, directory, model, time_updated FROM session WHERE directory LIKE ? AND time_archived IS NULL ORDER BY time_updated DESC LIMIT 5',
  ).all(`${projectPath}/%`)[0] || null;
}

/**
 * opencode 세션 상태 감지. Claude Code jsonl 없이 opencode만 돌려도
 * 세션 탭 상태 신호(busy/waiting/idle)가 뜨게 하는 보완 감지기.
 * @param {{id: string, path: string}} project
 * @returns {SessionState|null} 감지 실패/세션 없음이면 null
 */
export function detectOpencodeState(project) {
  const dbPath = opencodeDbPath();
  if (!existsSync(dbPath)) return null;
  let db;
  try { db = new DatabaseSync(dbPath, { readOnly: true }); }
  catch { return null; } // db locked or unreadable
  try {
    const row = opencodeSessionRow(db, project.path);
    if (!row) return null;
    const ageMs = Date.now() - Number(row.time_updated);
    let state = 'idle';
    if (ageMs < 15000) {
      state = 'busy';
      ...
      try {
        if (last && JSON.parse(last.data).role === 'assistant') state = 'waiting';
      } catch { /* malformed data blob — treat as idle */ }
    } else {
      return null; // 오래된 세션 — Claude 감지 결과에 방해되지 않도록 null
    }
    let model = null;
    try { model = JSON.parse(row.model || 'null')?.id || null; } catch { /* ignore */ }
    return {
      projectId: project.id,
      state,
      sessionId: row.id,
      lastActivity: new Date(Number(row.time_updated)).toISOString(),
      model,
    };
  } catch {
    return null;
...
/**
 * Merge Claude Code jsonl 상태와 opencode SQLite 상태 — 더 활동적인 쪽 선택.
 */
function pickMoreActive(a, b) {
  if (!a) return b || null;
  if (!b) return a;
  const rankA = STATE_RANK[a.state] || 0;
  const rankB = STATE_RANK[b.state] || 0;
  if (rankA !== rankB) return rankA > rankB ? a : b;
  if (a.state === 'idle') return a; // 둘 다 idle이면 어느 쪽이든 무의미
  return (Date.parse(b.lastActivity) || 0) > (Date.parse(a.lastActivity) || 0) ? b : a;
}
 ...
 */
function detectClaudeJsonlState(project) {
...
/**
 * Detect session state across Claude Code (jsonl) and opencode (SQLite).
 * @param {{id: string, path: string}} project
 * @returns {SessionState}
 */
export function detectSessionState(project) {
  return pickMoreActive(detectClaudeJsonlState(project), detectOpencodeState(project));
}

/**
 * Get list of sessions for a project (Claude Code jsonl + opencode SQLite, merged).
 * @param {{id: string, path: string}} project
 * @param {number} [limit=10]
 * @returns {SessionInfo[]}
 */
export function getProjectSessions(project, limit = 10) {
  const claude = getClaudeSessions(project, limit);
  const opencode = getOpencodeSessions(project, limit);
  return [...claude, ...opencode]
    .sort((a, b) => new Date(b.lastModified) - new Date(a.lastModified))
    .slice(0, limit);
}

function getClaudeSessions(project, limit) {
  const dir = getProjectJsonlDir(project.path);
  if (!dir) return [];
...
/**
 * opencode 세션 목록. sessionId는 Claude 세션(HEX/UUID)과 충돌하지 않도록
 * `oc:` 접두어를 붙인다 — messages/timeline/resume 라우트의 Claude 전용
 * sessionId 검증을 자연히 통과하지 못해 잘못된 재개가 차단된다.
 */
function getOpencodeSessions(project, limit) {
  const dbPath = opencodeDbPath();
  ...
  try {
    const rows = db.prepare(
      'SELECT id, title, time_updated FROM session WHERE (directory = ? OR directory LIKE ?) AND time_archived IS NULL ORDER BY time_updated DESC LIMIT ?',
    ).all(project.path, `${project.path}/%`, limit);
    return rows.map(row => ({
      sessionId: `oc:${row.id}`,
      lastModified: new Date(Number(row.time_updated)).toISOString(),
      sizeKB: 0,
      source: 'opencode',
      model: 'OpenCode',
...
export function discoverProjects(existingPaths) {
  ...
  // 0. Scan common project parent dirs for git repos (filesystem)
  try {
    const home = homedir();
    const roots = [
      'projects', 'Projects', 'code', 'Code', 'dev', 'Dev',
      'repos', 'workspace', 'Workspace', 'src', 'work', 'git'
    ].map(n => join(home, n));
    if (process.env.COCKPIT_PROJECT_ROOTS) {
      const sep = IS_WIN ? ';' : ':';
      roots.push(...process.env.COCKPIT_PROJECT_ROOTS.split(sep).filter(Boolean));
    }
    const MAX_DEPTH = 4;
    const SKIP = new Set(['node_modules', '.git', '.svn', '.hg', '.idea', '.vscode',
    ...
    function scan(dir, depth) {
      ...
      // Found a git repo here — register and stop descending
      if (entries.some(e => e.name === '.git')) {
        const p = dir.replace(/\\/g, '/');
        const key = p.toLowerCase();
        if (!existingSet.has(key) && !discovered.has(key)) {
          discovered.set(key, { path: p, name: dir.split(/[/\\]/).pop(), source: 'filesystem' });
        }
        return;
      }
    ...
    for (const root of roots) {
      try { if (statSync(root).isDirectory()) scan(root, 0); } catch { /* root absent */ }
    }
  } catch { /* ignore */ }
  ...
  // 3. Scan WSL distributions for Claude projects (Windows host only —
  // on Linux/macOS/WSL itself, native paths are already handled by step 0/2).
  if (IS_WIN) try {
  ...
  for (const [, info] of discovered) {
    ...
    const fsPath = IS_WIN ? path.replace(/\//g, '\\') : path;
    let pathExists = false;
    let hasGit = false;
    try {
      statSync(fsPath);
      pathExists = true;
      try { statSync(join(fsPath, '.git')); hasGit = true; } catch { /* not a git repo */ }
