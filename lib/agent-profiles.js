...
// ─── Tool Sets ───
export const ALL_TOOLS = new Set(['BASH', 'READ', 'SEARCH', 'EDIT', 'WRITE', 'GLOB', 'GIT_DIFF', 'GIT_LOG', 'JIRA', 'CICD', 'OPEN', 'COCKPIT', 'WEATHER', 'DELEGATE', 'CONSULT']);
// Intern gets minimal tools
export const INTERN_TOOLS = new Set(['OPEN', 'WEATHER', 'COCKPIT', 'READ', 'DELEGATE', 'CONSULT']);
// CEO gets everything
...
export const TEAMS = {
  dev: {
    ...
    tools: ['BASH', 'READ', 'SEARCH', 'EDIT', 'WRITE', 'GLOB', 'GIT_DIFF', 'GIT_LOG', 'CICD', 'COCKPIT'],
  ...
  design: {
    ...
    tools: ['READ', 'WRITE', 'EDIT', 'GLOB', 'SEARCH', 'OPEN', 'COCKPIT'],
    desc: 'UI/UX 가이드, 스타일 가이드, CSS 수정, 디자인 피드백',
  },
  admin: {
  ...
  marketing: {
    ...
    tools: ['WRITE', 'READ', 'EDIT', 'OPEN', 'SEARCH', 'COCKPIT', 'GIT_LOG', 'GLOB'],
    desc: '콘텐츠 생성/수정, 카피라이팅, SEO, 랜딩페이지, 릴리즈 노트',
  },
};
...
특징: Jira 관리, 노트/문서 작성, PRD, 스프린트 관리. 체계적이고 꼼꼼함.`,
  ...
  plan_daeri: {
    ...
    emoji: '📊', provider: 'gemini', model: 'gemini-2.5-pro', maxIter: 6,
...
특징: UI/UX 가이드, 디자인 리뷰, 스타일 가이드 관리. 심미적 판단.`,
  ...
  design_daeri: {
    ...
    emoji: '✨', provider: 'gemini', model: 'gemini-2.5-pro', maxIter: 5,
...
특징: 토큰/비용 추적, 시스템 모니터링, 리포트 생성. 숫자에 강함.`,
  ...
  admin_daeri: {
    id: 'admin_daeri', name: '차경리', rank: 'Asst.Mgr', team: 'admin', color: '#2dd4bf',
    emoji: '📑', provider: 'gemini', model: 'gemini-2.5-pro', maxIter: 5,
    persona: `너는 콕핏 경영지원 대리 "차경리"야.
말투: "분석 자료 정리했어요.", "비용 추이를 보면요..", "리포트 초안 만들어볼게요."
특징: 비용 분석, 시스템 리포트 작성, 데이터 정리. 꼼꼼한 숫자 분석.`,
  },
  admin_sawon: {
...
특징: 카피라이팅, 콘텐츠 전략, SEO, 랜딩페이지 기획. 직관적 판단.`,
  ...
  mkt_daeri: {
    id: 'mkt_daeri', name: '류마리', rank: 'Asst.Mgr', team: 'marketing', color: '#fb923c',
    emoji: '🎯', provider: 'gemini', model: 'gemini-2.5-pro', maxIter: 5,
    persona: `너는 콕핏 마케팅팀 대리 "류마리"야.
말투: "타겟 분석해봤어요.", "이 카피 어떨까요?", "트렌드 리서치 결과에요."
특징: 콘텐츠 분석, 카피 초안 리뷰, 트렌드 리서치, SEO 키워드 분석. 데이터 기반 마케팅.`,
  },
  mkt_sawon: {
...
특징: 콘텐츠 초안, 키워드 리서치, SNS 문구. 빠른 초안 생산.`,
  ...
  dev_reviewer: {
    ...
    emoji: '🔍', provider: 'claude', model: 'claude-sonnet-4-6', maxIter: 8,
...
특징: React 코드 리뷰 전문. 안티패턴 탐지, 성능 최적화 제안, 컨벤션 준수 확인.`,
  ...
  dev_security: {
    ...
    emoji: '🛡️', provider: 'claude', model: 'claude-sonnet-4-6', maxIter: 6,
...
// ─── Rank hierarchy for deliberation ───
export const RANK_LEVEL = {
  'Intern': 0, 'Staff': 1, 'Asst.Mgr': 2, 'Manager': 3,
  'Team Lead': 4, 'VP': 5, 'Director': 6,
};

/** Get numeric rank level for comparison */
export function getRankLevel(agentId) {
  const p = AGENT_PROFILES[agentId];
  return p ? (RANK_LEVEL[p.rank] ?? 0) : 0;
}

/** Find common superior for two agents.
 *  Same team → team's highest rank above both.
 *  Different teams → 이사(daepyo). */
export function findCommonSuperior(agentIdA, agentIdB) {
  const a = AGENT_PROFILES[agentIdA];
  const b = AGENT_PROFILES[agentIdB];
  if (!a || !b) return AGENT_PROFILES.daepyo;

  if (a.team && a.team === b.team) {
    const members = getTeamMembers(a.team);
    const rankA = getRankLevel(agentIdA);
    const rankB = getRankLevel(agentIdB);
    const maxRank = Math.max(rankA, rankB);
    // Find someone above both
    const superior = members
      .filter(m => getRankLevel(m.id) > maxRank && m.id !== agentIdA && m.id !== agentIdB)
      .sort((x, y) => getRankLevel(x.id) - getRankLevel(y.id))[0];
    return superior || AGENT_PROFILES.daepyo;
  }
...
export function getTeamMembers(teamId) {
  // Deduplicate — legacy aliases (sawon/daeri/gwajang/bujang) reference same objects
  const seen = new Set();
  return Object.values(AGENT_PROFILES).filter(p => {
    if (p.team !== teamId || seen.has(p.id)) return false;
    seen.add(p.id);
    return true;
  });
}
...
export function pickAgentByComplexity(teamId, complexity = 'low') {
  ...
  if (complexity === 'mid') {
    // For teams with 4+ members, round-robin between middle ranks to avoid idle agents
    const midMembers = members.slice(1, -1); // exclude highest and lowest
    if (midMembers.length > 1) {
      _midRoundRobin = (_midRoundRobin + 1) % midMembers.length;
      return midMembers[_midRoundRobin];
    }
    return midMembers[0] || members[Math.min(1, members.length - 1)] || null;
  }
...
export function getAgentTools(agentProfile) {
  ...
  return new Set([...team.tools, 'WEATHER', 'OPEN', 'DELEGATE', 'CONSULT']); // common tools added
}
