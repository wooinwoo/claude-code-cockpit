...
const _agentEvents = new Map();
const _agentContexts = new Map();

function compactPrompt(event) {
  const value = event.prompt ?? event.message ?? event.input?.prompt;
...
export function recordAgentEvent(event) {
  ...
  const termId = event.term_id || null;
  if (!sessionId && !termId) return null;
  const type = event.hook_event_name || event.type || 'Event';
  const key = termId || sessionId;
  const now = Date.now();
  if (type === 'SessionStart') { _agentContexts.delete(key); _summaries.delete(key); }
  if (type === 'UserPromptSubmit') {
    const prompt = compactPrompt(event);
    const previous = _agentContexts.get(key);
    if (prompt) _agentContexts.set(key, { goal: previous?.goal || prompt, updatedAt: now });
  }
  const state = type === 'PermissionRequest' || type === 'Notification' ? 'waiting'
    : type === 'Stop' || type === 'SessionEnd' ? 'done'
    : type === 'SessionStart' || type === 'TeammateIdle' ? 'idle' : 'busy';
  const record = {
    sessionId, termId, type, state,
    kind: event.agent_kind || null,
    cwd: event.cwd || null,
    reason: event.tool_input?.description || event.message || event.tool_name || null,
    updatedAt: now,
  };
  _agentEvents.set(key, record);
  return record;
}

export function getAgentEvents(now = Date.now()) {
  for (const [key, event] of _agentEvents) {
    if (now - event.updatedAt > 24 * 60 * 60_000) _agentEvents.delete(key);
  }
  return [..._agentEvents.values()];
}

// ─── 관제 타일 활동 요약 상태 ("지금 뭐 하는 중") ───
// mark = 터미널 누적 출력 카운터. 같은 mark 면 새 출력이 없다는 뜻이라 LLM 호출을 건너뜀.
const _summaries = new Map(); // termId → { text, at, mark, pending }
export function summaryGate(termId, mark, now = Date.now(), minIntervalMs = 20_000) {
  const s = _summaries.get(termId);
  if (!s) return true;
  if (s.pending && now - s.at < 60_000) return false;
  if (s.mark === mark) return false;
  return now - s.at >= minIntervalMs;
}

export function beginSummary(termId, mark, now = Date.now()) {
  const prev = _summaries.get(termId);
  _summaries.set(termId, { text: prev?.text || null, mark, pending: true, at: now });
}

export function endSummary(termId, text, now = Date.now()) {
  const s = _summaries.get(termId);
  if (!s) return;
  _summaries.set(termId, { ...s, text: text || s.text, pending: false, at: now });
}

export function getSummaries(now = Date.now()) {
  for (const [key, s] of _summaries) {
    if (now - s.at > 30 * 60_000) _summaries.delete(key);
  }
  for (const [key, context] of _agentContexts) {
    if (now - context.updatedAt > 24 * 60 * 60_000) _agentContexts.delete(key);
  }
  const keys = new Set([..._summaries.keys(), ..._agentContexts.keys()]);
  return Object.fromEntries([...keys].map(key => {
    const summary = _summaries.get(key);
    const context = _agentContexts.get(key);
    return [key, {
      text: summary?.text || null,
      goal: context?.goal || null,
      at: Math.max(summary?.at || 0, context?.updatedAt || 0),
    }];
  }).filter(([, value]) => value.text || value.goal));
}
