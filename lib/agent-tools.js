...
import { join, dirname, resolve, normalize, isAbsolute, basename } from 'node:path';
...
import * as telegram from './telegram-bridge.js';

const __dirname = dirname(fileURLToPath(import.meta.url));

// ─── 위험 도구 분류 (실행 전 텔레그램 승인 필요) ───
// hard: true → 신뢰 모드(/trust) 중에도 항상 확인. 영구 데미지/배포/sudo 등.
// hard: false → 신뢰 모드 활성 시 자동 승인. 일반 위험 (push, terminal-input 등).

const COCKPIT_RISKS = new Map([
  ['terminal-input',  { label: '🔌 터미널에 명령 주입',  hard: false }],
  ['terminal-create', { label: '🆕 새 터미널 생성',      hard: false }],
  ['git-push',        { label: '🚀 git push (원격)',     hard: false }],
  ['workflow-start',  { label: '⚙️ 워크플로 실행',      hard: false }],
]);

// BASH 경로의 위험 패턴. 순서 중요 — 더 구체적인 것을 먼저 매치.
const BASH_RISKS = [
  { pat: /\brm\s+-rf?\s+\/(?:\s|$)/,                       label: '☠️ rm -rf / (루트 삭제)',     hard: true  },
  { pat: /:\(\)\s*\{\s*:\|:&\s*\}/,                        label: '💣 fork bomb',                hard: true  },
  { pat: /\bgit\s+push\s+(--force\b|-f\b)/,                label: '💥 git push --force',         hard: true  },
  { pat: /\b(npm|pnpm|yarn)\s+(un)?publish\b/,             label: '📦 publish/unpublish',        hard: true  },
  { pat: /\bsudo\s+rm\b/,                                  label: '🛡 sudo rm',                  hard: true  },
  { pat: /\bdd\s+if=.*of=\/dev\//,                         label: '💾 dd to device',             hard: true  },
  { pat: /\bDROP\s+TABLE\b/i,                              label: '🗃 DROP TABLE',               hard: true  },
  { pat: /\bcurl\b.*\|\s*(bash|sh|zsh)\b/,                 label: '🌐 curl | shell (원격 실행)', hard: true  },
  { pat: /\bgit\s+reset\s+--hard\b/,                       label: '⚠️ git reset --hard',         hard: false },
  { pat: /\brm\s+-rf?\b/,                                  label: '🗑 rm -rf',                   hard: false },
  { pat: /\bsudo\b/,                                       label: '🛡 sudo',                     hard: false },
  { pat: /\bgit\s+push\b/,                                 label: '🚀 git push',                 hard: false },
  { pat: /\bkubectl\s+(delete|apply|exec)\b/,              label: '☸️ kubectl delete/apply/exec', hard: false },
  { pat: /\bdocker\s+(rm|kill|stop|exec)\b/,               label: '🐳 docker rm/kill/stop/exec', hard: false },
  { pat: /\baws\s+\S+\s+(delete|terminate|modify|put|create)/, label: '☁️ aws 변경 명령',       hard: false },
];

function classifyRisk(type, arg) {
  if (type === 'COCKPIT') {
    const sub = String(arg || '').split(':')[0];
    return COCKPIT_RISKS.get(sub) || null;
  }
  if (type === 'BASH') {
    const cmd = String(arg || '');
    for (const r of BASH_RISKS) {
      if (r.pat.test(cmd)) return { label: r.label, hard: r.hard };
    }
  }
  return null;
}
...
/** Sprint worktree paths — registered by sprint-engine, unregistered on cleanup */
const _worktreeRoots = new Set();
export function registerWorktreeRoot(path) { _worktreeRoots.add(resolve(normalize(IS_WIN ? toWinPath(path) : path))); }
export function unregisterWorktreeRoot(path) { _worktreeRoots.delete(resolve(normalize(IS_WIN ? toWinPath(path) : path))); }

/** Get ALL project roots as Windows-accessible paths (includes sprint worktrees) */
function getAllRootsFs() {
  const roots = _getProjectRoots ? _getProjectRoots() : [];
  const result = roots.map(r => resolve(normalize(IS_WIN ? toWinPath(r) : r)));
  // Worktree roots go FIRST so relative paths resolve to worktree before original project
  for (const wt of _worktreeRoots) result.unshift(wt);
  return result;
}
 ...
 *  M2: Allows sprint worktree paths (parent-dir/{projectName}-sprint-{id}) */
function resolveAndValidate(inputPath) {
  ...
  if (isAbsolute(inputPath)) {
    ...
    // 1st pass: Check against registered roots
    ...
    // 2nd pass: Check for sprint worktree pattern
    // Pattern: {parent}/{projectName}-sprint-{sprintId}/*
    // Example: C:/project/my-app-sprint-abc123/src/file.ts
    // where C:/project/my-app is a registered root
    for (const rootFs of allRoots) {
      const projectName = basename(rootFs);
      const projectParent = dirname(rootFs);
      const sprintPattern = new RegExp(
        `^${projectParent.replace(/\\/g, '/').toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/${projectName.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}-sprint-`
      );
      if (sprintPattern.test(resolvedNorm)) {
        return { valid: true, resolved, root: rootFs };
      }
    }

    return { valid: false, resolved: resolve(normalize(inputPath)), error: `Path outside registered projects: ${inputPath}` };
  }

  // For relative paths, try each root
  for (const rootFs of allRoots) {
    const resolved = resolve(rootFs, inputPath);
    const resolvedNorm = resolved.replace(/\\/g, '/').toLowerCase();
    const rootNorm = rootFs.replace(/\\/g, '/').toLowerCase();
    if (resolvedNorm.startsWith(rootNorm + '/') || resolvedNorm === rootNorm) {
      return { valid: true, resolved, root: rootFs };
  ...
  return { valid: false, resolved: '', error: `Cannot resolve path: ${inputPath}` };
}
...
/** Sprint-mode safety: allows npm/git/build commands but blocks truly dangerous ops */
function isSprintSafeCommand(cmd) {
  const lower = cmd.toLowerCase().trim();
  // Block absolutely dangerous operations even in sprint mode
  const hardBlock = [
    /\brm\s+-rf\s+[\/\\]/, /\bformat\b/, /\bmkfs\b/, /\bdd\b.*\bif=/, /:\(\)\s*\{/,
    /\bshutdown\b/, /\breboot\b/, /\bsudo\b/, /\buseradd\b/, /\bpasswd\b/,
    /\biptables\b/, /\bmount\b/, /\bpowershell\b.*-enc/, /\bcertutil\b/,
    /\bnet\s+user\b/, /\breg\s+(delete|add|import)\b/,
    /\bcurl\b.*-[oO]\b/, /\bwget\b/,
    /[`]/, /\$\(/, /\$\{/, /%[a-z]/i,
  ];
  for (const p of hardBlock) { if (p.test(lower)) return false; }
  return true;
}

async function executeBash(cmd, context) {
  const isSprintMode = context?.sprintMode === true;
  if (!isSprintMode && !isSafeCommand(cmd)) return 'Error: 이 명령어는 안전 필터에 차단됐어영. 읽기 전용 명령만 가능해여 (ls, dir, cat, type, git log, git status, git diff 등).';
  if (isSprintMode && !isSprintSafeCommand(cmd)) return 'Error: 이 명령어는 스프린트에서도 허용되지 않아영.';

  // Sprint mode: use worktree root as cwd, regular mode: use primary project root
  let root;
  if (isSprintMode && _worktreeRoots.size > 0) {
    root = [..._worktreeRoots][0]; // Use the first (most recently registered) worktree
  } else {
    root = getPrimaryRoot();
  }
  ...
  const { stdout, stderr } = await shellExec(root, cmd, {
    timeout: isSprintMode ? 120000 : BASH_TIMEOUT, // Sprint commands may take longer (npm install etc)
...
async function executeEdit(rawArg, context) {
  const lines = rawArg.split('\n');
  const filePath = lines[0].trim();
  console.error(`[TOOL] EDIT called: ${filePath}`);
  ...
  // Normalize line endings (\r\n → \n) for both content and old_content
  content = content.replace(/\r\n/g, '\n');
  const oldNorm = oldContent.replace(/\r\n/g, '\n');

  if (!content.includes(oldNorm)) {
    // Fallback: trim trailing whitespace per line
    const trimLine = s => s.split('\n').map(l => l.trimEnd()).join('\n');
    if (!trimLine(content).includes(trimLine(oldNorm))) {
      const oldLines = oldNorm.split('\n').slice(0, 3);
      const fileSnippet = content.slice(0, 300);
      console.log(`[EDIT DEBUG] MISMATCH in ${filePath}`);
      console.log(`[EDIT DEBUG] old_content(first3): ${JSON.stringify(oldLines)}`);
      console.log(`[EDIT DEBUG] file(first300): ${JSON.stringify(fileSnippet)}`);
      return 'Error: OLD_CONTENT 블록이 파일에서 찾을 수 없어영. READ로 정확한 내용을 다시 확인해주세영.';
    }
    // Matched with trimmed lines — proceed with trimmed version
  ...
  const newFileContent = content.replace(oldNorm, newNorm);
...
async function executeWrite(rawArg, context) {
  const lines = rawArg.split('\n');
  const filePath = lines[0].trim();
  console.error(`[TOOL] WRITE called: ${filePath} (${lines.length} lines)`);
  if (!filePath) return 'Error: 파일 경로가 필요해영.';
  ...
  if (exists && !isSprintMode) {
    return 'Error: 파일이 이미 있어영. 기존 파일을 수정하려면 EDIT를 사용하세영.';
  }
  // Sprint/AutoBuild mode: backup existing file before overwrite
  if (exists && isSprintMode) {
    try {
      mkdirSync(EDIT_BACKUP_DIR, { recursive: true });
      const safeName = filePath.replace(/[/\\:*?"<>|]/g, '_');
      copyFileSync(p, join(EDIT_BACKUP_DIR, `${safeName}_${Date.now()}.bak`));
  ...
  try {
    ...
    return `✅ ${filePath} ${verb} 완료 (${lineCount}줄, ${content.length}자).`;
  } catch (err) {
...
export async function executeTool(type, arg, context) {
  // 위험 도구는 사용자 텔레그램 승인 필요 (fail-closed)
  // hard=true → 신뢰 모드 중에도 확인. hard=false → 신뢰 모드 시 자동 승인.
  const risk = classifyRisk(type, arg);
  if (risk) {
    const ok = await telegram.requestApproval(risk.label, `${type}:${arg}`, { hard: risk.hard });
    if (!ok) {
      return `Error: 사용자가 거부했거나 60초 안에 응답이 없었습니다. (${risk.label})`;
    }
  }

  switch (type) {
    case 'BASH':     return executeBash(arg, context);
    case 'READ':     return executeRead(arg);
    case 'SEARCH':   return executeSearch(arg);
    case 'EDIT':     return executeEdit(arg, context);
    case 'WRITE':    return executeWrite(arg, context);
