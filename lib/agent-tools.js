...
import { AGENT_PROFILES, pickAgentByComplexity } from './agent-profiles.js';
...
export function initTools({ getProjectRoots, getProjectsMeta, getJiraConfig, cockpitServices, geminiApiKey, broadcast }) {
...
async function executeDelegateTask(arg, context) {
  ...
  try {
    ...
    const result = await _runSubAgentLoop(
      ...
      `${callerName}이(가) 위임한 작업입니다.`,
      taskDesc,
      context.loopState || { aborted: false }
    );
