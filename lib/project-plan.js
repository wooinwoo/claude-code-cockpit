  ...
  if (plan.worktreePath) {
    try {
      ...
      if (isIntegration && files) {
        const srcFiles = files.split('\n').filter(f => /\.(tsx?|ts)$/.test(f) && f.startsWith('src/'));
        const exports = [];
...
${actualFiles ? `## 현재 프로젝트에 존재하는 파일\n\`\`\`\n${actualFiles}\n\`\`\`\n` : ''}
${exportSummary ? `## 사용 가능한 컴포넌트/함수 (실제 export 목록)\n${exportSummary}\n\n⚠️ 위 export를 그대로 사용하세요. 존재하지 않는 모듈을 import하지 마세요.\n` : ''}

## 수행할 작업
${isIntegration ? `⚠️ 이 Feature는 통합(integration) 작업입니다.
**절대 규칙**: 위 "사용 가능한 컴포넌트/함수" 목록에 있는 것만 import하세요.
존재하지 않는 ThemeProvider, ThemeToggle 등을 만들어내지 마세요.

1. 위 export 목록을 확인하고, 그 파일들만 import하세요
2. import 경로는 상대경로(./components/TodoForm 등)를 사용하세요
3. WRITE로 App.tsx를 완전히 새로 작성하세요 (기존 파일 덮어쓰기)
4. 완료 후 git add -A` : `1. 기존 코드 확인: GLOB src/**/*
...
- import 경로는 반드시 실제 파일 위치에 맞추세요 — 존재하지 않는 경로 import 금지
