...
// ─── Agent Templates ───
const AGENTS = {
  architect: { id: 'dev_bujang', name: '김부장', emoji: '📋', provider: 'gemini', model: 'gemini-2.5-pro' },
  senior: { id: 'dev_gwajang', name: '원과장', emoji: '😎', provider: 'gemini', model: 'gemini-2.5-pro' },
  mid: { id: 'dev_daeri', name: '핏대리', emoji: '🔥', provider: 'gemini', model: 'gemini-2.5-flash' },
  junior: { id: 'dev_sawon', name: '콕사원', emoji: '🐣', provider: 'gemini', model: 'gemini-2.5-flash' },
};
...
class AutoBuildPlan {
  ...
  toJSON() {
    return {
      ...
      baseBranch: this.baseBranch, toolchain: this.toolchain,
...
export async function generatePlan(projectId, description, images = []) {
  ...
  // Always use Gemini for plan generation — Claude times out on structured JSON tasks
  const profile = {
    id: 'plan_analyst', name: '플래너', emoji: '📋',
    provider: 'gemini', model: 'gemini-2.5-pro',
    team: 'dev', maxIter: 1, systemPrompt: '',
  };
...
async function _runAutoBuildPipeline(plan) {
  ...
  try {
    ...
    // ── Phase 2: Final validation (skipped) ──
    plan.currentPhase = 'final-validation';
    plan.addLog('system', '⏭️ Final validation skipped', '');
    _broadcastProgress(plan);
    // await _validateAndHeal(plan);
    // if (plan.isStopped()) return _finalizePlan(plan, 'paused');
...
async function _executeSingleFeature(plan, feature) {
  ...
  // Pick agent based on feature complexity
  const agentKey = feature.files.length > 3 ? 'senior' : 'mid';
...
2. READ로 해당 파일 확인
3. EDIT로 최소한의 수정만 (다른 파일 건드리지 마세요)
...
async function _setupWorktree(plan) {
  ...
  } catch { /* ok, default to main */ }

  plan.addLog('system', `기본 브랜치: ${plan.baseBranch}`, '');

  // Fetch + create worktree
  const fetchResult = await _execInProject(projectPath, `git fetch origin ${plan.baseBranch}`);
  if (fetchResult.exitCode !== 0) throw new Error(`git fetch failed: ${fetchResult.output.slice(0, 200)}`);
  plan.addLog('system', `워크트리 생성: ${worktreePathFwd}`, '');
  await _execInProject(projectPath, `git worktree add ${worktreePathFwd} -b ${plan.branch} origin/${plan.baseBranch}`);
...
function _buildFeaturePrompt(plan, feature) {
  ...
  const doneDepInfo = feature.deps
    ...
    .map(f => `- ${f.id}: ${f.title} (files: ${f.files.join(', ')})`)
    .join('\n');

  const rules = plan.toolchain ? _getToolchainRules(plan.toolchain) : '';
...
${doneDepInfo ? `## 완료된 선행 Feature\n${doneDepInfo}\n` : ''}

## 수행할 작업
1. 기존 코드 확인: GLOB src/**/* 또는 해당 디렉토리
2. ${feature.type === 'modify' ? '기존 파일 READ → EDIT로 수정' : 'WRITE로 새 파일 생성'}
...
- Tailwind CSS 유틸리티 클래스만 (CSS 파일 작성 금지)
- shadcn/ui 우선 사용
- 200줄 넘는 파일은 분리
- useEffect deps 빠뜨리지 말 것
- 다른 Feature의 파일을 덮어쓰지 마세요 — 수정만 허용
- git commit 하지 마세요 — git add -A만 해주세요`;
}

function _getToolchainRules(toolchain) {
  const rules = {
...
- UI: shadcn/ui 우선 (Button, Card, Dialog 등)
  ...
  "features": [
    {
      "id": "F-1",
      "title": "프로젝트 초기 설정",
      "description": "Tailwind, shadcn/ui 설정. tsconfig paths alias 추가. src/shared/lib/utils.ts 생성.",
      "phase": "setup",
      "deps": [],
      "files": ["tailwind.config.ts", "src/shared/lib/utils.ts"],
      "type": "create"
    },
    {
      "id": "F-2",
      "title": "공통 레이아웃",
      "description": "Header, Sidebar, MainLayout 컴포넌트 구현...",
      "phase": "setup",
      "deps": ["F-1"],
      "files": ["src/widgets/layout/Header.tsx", "src/widgets/layout/Sidebar.tsx", "src/app/layout.tsx"],
      "type": "create"
    }
  ]
}
...
function _parseFeatureList(response) {
  ...
  while ((fm = fenceRe.exec(jsonStr)) !== null) {
    ...
    if (!p) continue;
    if (p.features?.length) return { phases: p.phases || _defaultPhases(), features: p.features };
