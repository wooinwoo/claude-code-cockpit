...
export function resumeSession(project, sessionId) {
  // Claude 세션 ID(HEX/UUID)만 --resume 대상. `oc:`(opencode) 등 다른 소스의 ID가
  // 흘러들어오면 잘못된 재개 대신 최근 세션 이어하기로 폴백한다.
  const resumable = /^[a-f0-9-]+$/i.test(sessionId || '');
  const claudeArgs = sessionId && resumable
    ? ['--resume', sessionId]
    : ['--continue'];
