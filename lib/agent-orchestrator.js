...
export async function detectMode(userMessage) {
  ...
  // ─── 이름 직접 호출 ───
  const nameCallMatch = msg.match(/(?:콕핏이사|김부장|핏대리|원과장|콕사원|한기장|박기리|이기원|최디장|정디리|강경장|윤경원|오마장|신마원|막내)/);
  if (nameCallMatch) {
    const nameMap = {
      '콕핏이사':'daepyo',
      ...
      '강경장':'admin_teamlead', '윤경원':'admin_sawon',
      '오마장':'mkt_teamlead', '신마원':'mkt_sawon', '막내':'intern',
    };
...
tool_calls 항목 형식:
  일반: {"tool":"도구명","argument":"인자"}  (도구명: BASH, READ, SEARCH, EDIT, WRITE, GLOB, GIT_DIFF, GIT_LOG, JIRA, CICD, OPEN, COCKPIT, WEATHER, DELEGATE)
...
export function buildProjectContext() {
  ...
  if (firstRoot) {
    try {
      const fsPath = IS_WIN ? toWinPath(firstRoot) : firstRoot;
      const entries = readdirSync(fsPath).slice(0, 30);
      ctx += `\n\n**${projects[0]?.name || 'Project'}** 루트 파일:\n${entries.join(', ')}`;
    } catch { /* ignore */ }

    try {
      const pkgPath = join(IS_WIN ? toWinPath(firstRoot) : firstRoot, 'package.json');
  ...
  try {
    while (iteration < maxIter) {
      ...
      for (let ti = 0; ti < parsed.toolCalls.length; ti++) {
        ...
        let toolResult = '';
        const toolContext = { convId, callerAgentId: profile.id, loopState };
        try {
          const toolTimeout = tc.type === 'DELEGATE' ? 120000 : (TOOL_TIMEOUTS[tc.type] || 60000);
...
export function buildOrchestratorPlanPrompt(userMessage) {
  ...
  const teamList = Object.values(TEAMS).map(t => `- ${t.id}: ${t.name} (${t.desc})`).join('\n');
  const agentList = Object.values(AGENT_PROFILES)
    .filter((v, i, a) => a.findIndex(x => x.id === v.id) === i && v.id !== 'daepyo')
    .map(a => `- ${a.id}: ${a.name} (${a.rank}, ${a.team || '무소속'})`)
    .join('\n');
...
JSON으로만 답변: {"tasks":[{"id":"t1","description":"...","assignee":"에이전트ID","deps":[]}]}`;
}
...
export async function runSubAgentLoop(convId, task, profile, depContext, originalMessage, parentLoopState) {
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    for (const tc of parsed.toolCalls) {
      ...
      let toolResult = '';
      const subToolContext = { convId, callerAgentId: profile.id, loopState: parentLoopState };
      try {
        const toolTimeout = tc.type === 'DELEGATE' ? 120000 : (TOOL_TIMEOUTS[tc.type] || 60000);
        toolResult = await Promise.race([
