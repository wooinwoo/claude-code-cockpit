...
import { toWinPath, shellExec } from './wsl-utils.js';
...
export async function reconcileImports(plan) {
  ...
  if (missingDeps.length > 0) {
    ...
    try {
      await shellExec(wt, `npm install ${missingDeps.join(' ')} --save 2>&1`, { timeout: 120000 });
...
function resolveImport(wtRoot, fromFile, importPath) {
  ...
  // Direct match (import path already includes extension)
  if (existsSync(join(wtRoot, target))) return target;
...
function createStub(wtRoot, importPath, importSpec, fromFile) {
  ...
  const stripped = target.replace(/\.(tsx?|jsx?)$/, '');
  const filePath = join(wtRoot, stripped + '.tsx');
