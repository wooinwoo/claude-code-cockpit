...
/**
 * Synchronous git for a project (WSL-aware). Same routing as gitExec but blocking.
 * Returns stdout as a string. Throws on non-zero exit.
 * @param {string} projectPath
 * @param {string[]} args
 * @param {import('node:child_process').ExecFileSyncOptionsWithStringEncoding} [opts]
 * @returns {string}
 */
export function gitExecSync(projectPath, args, opts = {}) {
  const wsl = parseWslPath(projectPath);
  const merged = { timeout: 30000, maxBuffer: 1024 * 1024, windowsHide: true, encoding: 'utf8', ...opts };

  if (wsl) {
    return execFileSync('wsl', ['-d', wsl.distro, '--cd', wsl.linuxPath, 'git', ...args], {
      ...merged,
      cwd: process.env.SYSTEMROOT || 'C:\\Windows'
    });
  }
  ...
  const cwd = IS_WIN ? toWinPath(projectPath) : projectPath;
  return execFileSync('git', ['-C', cwd, ...args], merged);
}
