...
export class PencilClient {
  ...
  async callTool(name, args = {}) {
    ...
    try {
      ...
      if (e.message?.includes('ession') || e.message?.includes('nvalid')) {
        console.warn(`[Pencil] Session error on ${name}, re-initializing...`);
        try {
          await this._initialize();
          const result = await this._rawRequest('tools/call', { name, arguments: args });
          ...
          console.error(`[Pencil] Re-init failed: ${e2.message?.slice(0, 200)}`);
          this.available = false;
          return null;
        }
      }
      console.error(`[Pencil] callTool(${name}) error:`, e.message?.slice(0, 300));
      return null;
    }
  }
