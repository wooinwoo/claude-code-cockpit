...
export class PencilClient {
    ...
    try {
      ...
      for (const page of pages) {
        try {
          ...
          console.log(`[Pencil] Designing page: ${page.name} (${ops.split('\n').length} operations)`);
          const designResult = await this.batchDesign(ops);
          console.log(`[Pencil] Design result:`, JSON.stringify(designResult)?.slice(0, 200));

          // Extract node ID from design result
          const nodeId = this._extractNodeId(designResult);
          console.log(`[Pencil] Created node: ${nodeId}`);
          ...
          if (nodeId && outputDir) {
            try {
              ...
              const exportResult = await this.callTool('export_nodes', { nodeIds: [nodeId], format: 'png', outputDir: exportFolder });
              console.log(`[Pencil] Export result:`, JSON.stringify(exportResult)?.slice(0, 200));
              // Read exported file
              const pngPath = join(outputDir, `${nodeId}.png`);
          ...
          // Fallback: try get_screenshot (only works if VS Code is visible)
          if (!ssData && !outputDir) {
            const ss = await this.getScreenshot(nodeId);
            ssData = ss?.content?.find(c => c.type === 'image')?.data || null;
          }
  ...
  _buildPageOperations(page, primaryColor, index) {
    ...
    // Tested: these operations are confirmed working with Pencil MCP
    return [
      `s${index}=I(document, {type:"frame", name:"${pageName}", layout:"vertical", width:390, height:844, x:${xOffset}, fill:"#F7F8FA", clip:true})`,
      `h${index}=I(s${index}, {type:"frame", layout:"horizontal", width:"fill_container", height:56, padding:[0,20], alignItems:"center", fill:"#FFFFFF"})`,
      `I(h${index}, {type:"text", content:"${pageName}", fontSize:20, fontWeight:"bold", fill:"#191F28"})`,
      `c${index}=I(s${index}, {type:"frame", layout:"vertical", width:"fill_container", height:"fill_container", padding:[16,20,80,20], gap:16})`,
      `b${index}=I(c${index}, {type:"frame", layout:"vertical", width:"fill_container", padding:[20,20], gap:8, cornerRadius:16, fill:"${primaryColor}"})`,
      `I(b${index}, {type:"text", content:"${desc || pageName}", fontSize:18, fontWeight:"bold", fill:"#FFFFFF"})`,
      `d${index}=I(c${index}, {type:"frame", layout:"horizontal", width:"fill_container", padding:16, gap:12, cornerRadius:16, fill:"#FFFFFF", stroke:{align:"inside", fill:"#E5E8EB", thickness:1}, alignItems:"center"})`,
      `I(d${index}, {type:"frame", width:48, height:48, cornerRadius:12, fill:"#E8F3FF"})`,
      `t${index}=I(d${index}, {type:"frame", layout:"vertical", width:"fill_container", gap:4})`,
      `I(t${index}, {type:"text", content:"${pageName} 항목", fontSize:15, fontWeight:"600", fill:"#191F28"})`,
      `I(t${index}, {type:"text", content:"상세 내용", fontSize:13, fill:"#8B95A1"})`,
    ].join('\n');
  }
