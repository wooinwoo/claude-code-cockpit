      ...
      for (let idx = 0; idx < pages.length; idx++) {
        ...
        try {
          await pageTimeout(async () => {
            ...
            // 2-pass design: layout+content → polish
            const passes = [
              {
                name: 'main',
...
- Left sidebar (240px) with navigation links to all pages, active state for current page
- Top header bar with page title, search input, user avatar
- Main content with cards, stats, tables, lists — filled with realistic Korean sample data
- Proper spacing, padding, typography hierarchy
- Use the style guide colors and typography exactly

Use 25 operations. Output ONLY operations.`
              },
              {
                name: 'polish',
                prompt: `ADD MORE CONTENT AND POLISH to "${pageName}" inside frame "${frameId}".

The basic structure exists. Now add:
- More data rows, table entries, list items with realistic Korean data
- Status badges (활성, 만료, 사용완료 etc.) with appropriate colors
- Icons (lucide) for navigation items and actions
- Secondary stats, charts placeholders, action buttons
- Any missing labels, descriptions, timestamps
