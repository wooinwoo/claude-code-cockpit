...
export class PencilClient {
  ...
  async callTool(name, args = {}) {
    ...
    try {
      ...
      console.error(`[Pencil] callTool(${name}) error:`, e.message);
      return null;
    }
  }
    ...
    try {
      ...
      for (const page of pages) {
        try {
          ...
          if (nodeId && outputDir) {
            try {
              const exportResult = await this.callTool('export_nodes', { nodeIds: [nodeId], format: 'png', folder: outputDir });
