...
- Output ONLY batch_design operations, one per line. No markdown, no explanation, no backticks.
- Variable names: ONLY lowercase a-z letters. NO digits, underscores, camelCase.
- Max 25 operations per response.
- All colors: 6-digit hex (#RRGGBB). Text MUST have fill property or it's invisible!
- The root frame is already created — use the given parentFrameId. Set placeholder:true on it first.
- Korean text for UI labels. Real content, not placeholder text.
- Icons: type:"icon_font", iconFontFamily:"lucide", iconFontName:"icon-name" (NOT "icon"!)
- Text: NEVER set width/height without textGrowth. Use textGrowth:"fixed-width" + width:"fill_container" for wrapping text.
- Use fill_container for children in flexbox layouts. Use fit_content for containers.
- Tables: Table frame → Row frame → Cell frame → Content (never skip cell frame).
- Images: NO image node type! Use G(frameId, "stock", "keywords") on a frame/rectangle.
- Remove placeholder:true when done with each frame.
- padding format: [top, right, bottom, left] or single number.`;

      // Step 3: Design each page
      const screenshots = [];
      for (let idx = 0; idx < pages.length; idx++) {
        const page = pages[idx];
        const pageTimeout = (fn) => Promise.race([fn(), new Promise((_, rej) => setTimeout(() => rej(new Error('Page design timeout (120s)')), 120000))]);
        try {
          await pageTimeout(async () => {
            ...
            // 2-pass design: structure+content → polish
            // First set placeholder on root frame
            try { await this.batchDesign(`U("${frameId}",{placeholder:true})`); } catch {}

            const passes = [
              {
                name: 'main',
                prompt: `Design the "${pageName}" page (route: ${page.route}).
Parent frame ID: "${frameId}" (placeholder:true is already set)
...
All pages: ${pages.map(p => p.name).join(', ')}

BUILD A COMPLETE WEBAPP PAGE:
1. First set layout on parent: U("${frameId}",{layout:"horizontal"})
2. Left sidebar (240px, vertical layout, dark or accent bg) with app name + nav links to all pages
3. Right main area (fill_container, vertical layout):
   a. Top header bar (64px) with page title text + search + avatar
   b. Content area with stat cards (horizontal row), data table with Korean sample data
4. All text MUST have fill:"#hex" or it's invisible
5. Icons use: type:"icon_font", iconFontFamily:"lucide", iconFontName:"home" (NOT "icon"!)
6. Use textGrowth:"fixed-width" + width:"fill_container" for text that should wrap

25 operations max. Output ONLY the operations, one per line.`
              },
              {
                name: 'polish',
                prompt: `ADD MORE CONTENT to "${pageName}" inside frame "${frameId}".

The basic layout exists. Now enrich it:
- Add more table rows with realistic Korean data (이름, 날짜, 상태 etc.)
- Status badges: frames with cornerRadius:9999, small padding, colored fill + text
- Action buttons in header or content area
- Secondary text, timestamps, descriptions
- Icons: type:"icon_font", iconFontFamily:"lucide", iconFontName:"chart-bar" etc.

Variable prefix: "${v}r" (use ${v}ra, ${v}rb, ${v}rc, etc.)
25 operations max. Output ONLY the operations.`
              }
            ];
            ...
            for (const pass of passes) {
              ...
              try {
                ...
                console.warn(`[Pencil] ${pageName}/${pass.name} failed: ${passErr.message?.slice(0, 100)}`);
              }
            }
