...
- All colors: 6-digit hex (#RRGGBB).
- The root frame is already created — use the given parentFrameId.
- Korean text for UI labels.
- Create realistic, production-quality UI with real content (not lorem ipsum).
- Include proper spacing, padding, typography hierarchy.
- Use "fill_container" for full-width elements.`;

      // Step 3: Design each page
      const screenshots = [];
      for (let idx = 0; idx < pages.length; idx++) {
        const page = pages[idx];
        const pageTimeout = (fn) => Promise.race([fn(), new Promise((_, rej) => setTimeout(() => rej(new Error('Page design timeout (300s)')), 300000))]);
        try {
          await pageTimeout(async () => {
            ...
            // Multi-pass design: structure → content → detail
            const passes = [
              {
                name: 'structure',
                prompt: `Design the LAYOUT STRUCTURE for "${pageName}" page (route: ${page.route}).
...
CREATE THE MAIN STRUCTURE ONLY:
- Sidebar navigation (if applicable) with links to all pages
- Top header bar with page title, search, user avatar
- Main content area with proper padding
- Key section containers (empty frames with proper sizing)

Use 25 operations max. Output ONLY operations.`
              },
              {
                name: 'content',
                prompt: `ADD CONTENT to the "${pageName}" page. The structure already exists inside frame "${frameId}".

Page purpose: ${page.description || pageName}
${page.components ? `Key components: ${page.components.join(', ')}` : ''}

NOW ADD THE ACTUAL CONTENT:
- Data cards with numbers, labels, icons
- Tables or lists with realistic Korean sample data
- Form inputs, buttons, status badges
- Charts placeholder frames
- Real text content (not lorem ipsum) — use Korean

Variable prefix: "${v}p" (use ${v}pa, ${v}pb, ${v}pc, etc.)
Insert into existing child frames of "${frameId}".
Use 25 operations max. Output ONLY operations.`
              },
              {
                name: 'detail',
                prompt: `ADD VISUAL POLISH to the "${pageName}" page inside frame "${frameId}".

ADD FINISHING TOUCHES:
- Icon decorations (lucide icons)
- Status indicators with colors
- Subtle dividers or spacing adjustments
- Any missing labels or secondary text
- Stock images where appropriate (use G() operation)

Variable prefix: "${v}q" (use ${v}qa, ${v}qb, ${v}qc, etc.)
Use 25 operations max. Output ONLY operations.`
              }
            ];
            ...
            for (const pass of passes) {
              ...
              try {
                ...
                const lines = cleaned.split('\n')
                  .map(l => l.trim())
                  .filter(l => l && /^[a-z]+=/.test(l) && (l.includes('I(') || l.includes('U(') || l.includes('G(') || l.includes('R(') || l.includes('C(')))
                  .slice(0, 25);
                if (lines.length > 0) {
