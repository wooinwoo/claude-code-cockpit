...
export const LIMITS = {
  jsonBodyBytes: 5 * 1024 * 1024,        // 5MB — max JSON request body
...
function deriveKey() {
  ...
  try { writeFileSync(KEY_FILE, key.toString('hex'), 'utf8'); } catch {}
  return key;
}
...
export function getAiConfig() {
  try {
    ...
    if (config?.geminiApiKey) {
      try { config.geminiApiKey = decryptToken(config.geminiApiKey); } catch {}
    }
    return config;
  } catch { return null; }
}
