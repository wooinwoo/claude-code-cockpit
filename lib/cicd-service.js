...
// ─── Workflow Runs ───
export async function getWorkflowRuns(projectPath, { workflow, status, branch, limit = 30 } = {}) {
  const args = ['run', 'list', '--json', 'databaseId,name,displayTitle,headBranch,headSha,status,conclusion,event,createdAt,updatedAt,url,workflowName', '-L', String(limit)];
  if (workflow) args.push('-w', workflow);
  if (status) args.push('-s', status);
  if (branch) args.push('-b', branch);
  return cached(`runs:${projectPath}:${workflow || ''}:${status || ''}:${branch || ''}`, async () => {
...
export async function rerunWorkflow(projectPath, runId, { failed = false } = {}) {
  ...
  for (const key of _cache.keys()) if (key.startsWith(`runs:${projectPath}:`)) _cache.delete(key);
  return { success: true };
}
